    ...
    <?php 
        switch (true){
            case ! is_numeric($min) || $min==0:
                print scriviMessaggio("Il valore minimo non è corretto", "danger");
                break;
            case ! is_numeric($max)  || $max==0:
                print scriviMessaggio("Il valore massimo non è corretto", "danger");
                break;
            case $min >= $max && $min != 0 && $max != 0:
                print scriviMessaggio("Il valore minimo non può essere uguale o superiore al massimo", "danger");
                break;
            default:
                if ($min > 0 && $max >0){
                    $numRnd = rand($min,$max);
                    print scriviMessaggio("<span class=\"display-5\">Il numero generato è $numRnd</span>", "info");
                }else{
                    print scriviMessaggio("Ritenta", "warning");
                }
                break;
        }       
        print $form;
...
<?php 
//     function scriviMessaggio($message, $type){
//         print <<<EOD
